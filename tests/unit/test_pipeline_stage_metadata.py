...
def test_build_notam_analysis_metadata_computes_cost() -> None:
    ...
    metadata = build_notam_analysis_metadata_from_batch(
        batch_result,
        input_cost_per_m=3.0,
        output_cost_per_m=15.0,
    )

    assert metadata.categorisation_batches == 1
    assert metadata.categorize_input_tokens == 1_000_000
    assert metadata.categorize_output_tokens == 1_000_000
    assert metadata.est_cost == pytest.approx(18.0)
    assert metadata.slowest_batch_ms == 100
    assert metadata.retried_category_notam_ids == []
