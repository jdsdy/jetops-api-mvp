...
from app.services.analysis.analysis_context import build_flight_context, build_notam_rows
...
from app.services.analysis.notam_heuristic_category import heuristic_category
...
def execute_notam_analysis_with_retries(
    flight,
    notam_rows: list[AnalysisNotamRow],
    *,
    settings: Settings | None = None,
    on_retry: Callable[[], None] | None = None,
) -> tuple[AnalysisJobResult, NotamAnalysisMetadata, list[int]]:
    if settings is None:
        settings = get_settings()

    heuristic_categories = _heuristic_categories(notam_rows)
    job_result = analyze_notam_job(flight, notam_rows, settings=settings)
    ...
    errored_raw_ids = _map_raw_notam_ids(notam_rows, still_pending) if still_pending else []
    ...
    return job_result, metadata, errored_raw_ids
